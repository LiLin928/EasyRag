...
>
> **实现说明（2026-08-28）**：实际未安装 `sse-starlette`，改用 FastAPI `StreamingResponse`（`media_type="text/event-stream"`）+ 手动 SSE 格式化（`event: {type}\ndata: {json}\n\n`），效果一致。
...
### 附录 E：Phase 1 实现进度（2026-08-28 更新）

#### 已完成

- **认证**：login / refresh / user-info / logout（`auth.py`）；LOGIN_FAILED 错误码移至 40010（避开 40100-40199 refresh 触发段）
- **知识库**：KB CRUD + 文档上传/删除/解析/结构树/元素列表（`knowledge.py`、`assets.py`、`tree.py`、`elements_list.py`、`parse_tasks.py`）
- **元数据**：KB 元数据字段 CRUD + 检索元数据过滤（`metadata.py`、`metadata_service.py`）
- **检索**：向量+全文+RRF+Rerank+导航 pipeline（`pipeline.py`）；检索设置 per-KB 解析（`retrieval_settings_service.py`）；检索测试工作台（`retrieval_testing.py`）
- **设置**：模型配置 CRUD + 分组响应 `{llm:[], embed:[], rerank:[]}` + `ModelResponse`（`settings.py`）；场景 CRUD + `GET /settings/scenes/:id`（`settings.py`）
- **对话（Plan 5）**：`POST /chat`（SSE）+ 会话 CRUD + 历史消息（`chat.py`、`chat_service.py`、`conversation.py` 模型）；SSE 事件对齐前端 `types/chat.ts`（phase/navigation/references/token/done/trace/error）
- **元素（Plan 6）**：`GET /elements/:id` 单元素详情 + `GET /elements/:id/context?window=N` 上下文（`elements.py`）
- **场景**：`GET /scenes` 顶层精简格式 `{id, name, desc}`（`scenes.py`，id=scene code）
- **反馈**：`POST /feedback` like/dislike（`feedback.py`、`Feedback` 模型）
- **引用构建**：`reference_builder.py` 输出含 `doc_title`/`node_title`（对齐前端 `Reference` 类型）
- **迁移**：`a3f2c8e1b9d4_conversations_messages_feedbacks.py`（conversations / messages / feedbacks 三表）

#### 命名约定决策

- **前端契约优先**：Pydantic schema 中直接使用前端字段名（camelCase），如 `messageId`、`doc_title`、`content_preview`，避免 alias 层带来的维护负担
- **KBOut / DocOut**：保留 snake_case ORM 字段 + camelCase 别名字段（`chunk_count`/`docCount`、`total_size`/`totalSize`），供前端直接消费
- **ModelResponse**：从 `params` JSONB 提取 `temp`/`ctx`/`dim`，`key` 掩码返回，分组聚合 `{"llm":[], "embed":[], "rerank":[]}`
- **Scene 双端点**：`/settings/scenes` 返回完整场景对象（含 config），`/scenes` 返回精简格式（仅 id/name/desc），id 为 scene code
...
- Langfuse 全链路追踪
- 代码沙箱池（Docker 容器）
- 生产环境 PostgresSaver checkpoint 持久化
- Redis pub/sub SSE 多 worker 分发
- ARQ 任务队列集成（工作流异步执行 + 重试）
- Webhook/API 触发器
- 工作流版本 diff 可视化

### Phase 2 实现进度（2026-08-28 更新）

#### CRUD 层（commit 908542d）

5 个 ORM 模型 + 5 个 Pydantic schemas + 8 个路由文件 + 1 个 Alembic 迁移 + 2 个注册更新：

- **工具（tools）**：CRUD + /test 执行端点 + config JSONB 列
- **技能（skills）**：CRUD + trigger/prompt/tools/docs/wfs/examples/scripts
- **MCP（mcps）**：CRUD + /test 连通测试端点 + env 环境变量
- **智能体（agents）**：CRUD + /:id/chat SSE 对话 + 五类资源挂载
- **工作流（workflows）**：CRUD + 发布/复制/执行 + 版本/执行/待办/模板
- **执行（executions）**：列表 + /stream SSE + cancel/resume + debug
- **待办（todos）**：列表 + submit（恢复工作流）/reject（标记失败）
- **模板（templates）**：列表 + /:id/instantiate 从模板创建

迁移：c4d7e9f2a1b3（九表）

#### 服务层（commit a53a12c）

**SSE 基础设施**：app/sse/emitter.py -- sse_event / sse_data

**工具系统**：
- core/tools/builtins.py -- 内置工具（current_time, string_length, uuid）
- core/tools/executor.py -- HTTP/builtin/Python 三类执行器
- services/tool_service.py -- execute_tool + get_tool

**技能系统**：services/skill_service.py -- match_skills + apply_skills

**MCP 客户端**：core/agent/tool_adapters/mcp_tools.py -- langchain-mcp-adapters

**Agent 系统**：
- core/agent/tool_registry.py -- build_tools 聚合五类资源为 BaseTool
- core/agent/memory.py -- get_checkpointer MemorySaver/PostgresSaver
- services/agent_service.py -- AgentService.chat create_react_agent + SSE

**工作流引擎（LangGraph）**：
- core/engine/state.py -- WorkflowState + resolve 变量插值
- core/engine/nodes/base.py -- BaseNodeExecutor + NodeRouter 工厂
- core/engine/nodes/basic.py -- 12 种节点执行器
- core/engine/sse_bus.py -- subscribe/publish/drain 单 worker Queue
- core/engine/graph_builder.py -- GraphBuilder.build -> CompiledStateGraph
- core/engine/executor.py -- execute_workflow + execute_workflow_sync

**路由连接**：
- tools /test -> execute_tool
- mcps /test -> test_connection
- agents /:id/chat -> AgentService SSE
- workflows /:id/execute -> execute_workflow
- executions /:id/stream -> SSE bus
- executions cancel/resume/debug -> 状态 + 引擎
- todos submit -> 恢复暂停工作流
- todos reject -> 标记执行 failed

**数据模型变更**：Tool 表新增 config JSONB 列（迁移 d5e8f1a3b2c4）

**新增依赖**：langgraph>=0.2, langchain-mcp-adapters>=0.1

#### 架构决策

- 变量插值：{{node_id.output.field}} / {{workflow.custom.x}} / {{loop.item}}
- 节点注册：NodeRouter.register 模块加载时自动注册
- SSE 事件：node_start/node_complete/execution_complete/execution_paused/error
- Agent SSE：phase/tool_start/tool_end/token/done/error（对齐 types/agent.ts）
- Human 节点：创建 Todo + paused，submit 触发 _try_resume_workflow
- 条件分支：ConditionExecutor 返回 _branch，add_conditional_edges 路由
- Loop 节点：loop_stack 长度判断迭代，_loop.item/index 注入

---
| 版本 | 日期 | 变更 |
|------|------|------|
| V1.0 | 2026-08-08 | 基于 Grill-Me 16 项决策 + V2 文档 45 项决策，产出后端工程化落地方案 |
| V1.1 | 2026-08-28 | Phase 1 全部端点实现完成；追加附录 E 实现进度与命名约定决策 |
| V1.2 | 2026-08-28 | Phase 2 CRUD + 服务层实现完成；追加 Phase 2 实现进度 |
