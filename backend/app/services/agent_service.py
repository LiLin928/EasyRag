...
class AgentService:
    ...
    async def chat(self, agent_id: str, question: str, user_id) -> AsyncIterator[str]:
        ...
        with traced_span(
            ...
            # 检查是否有历史状态
            with traced_span("agent.load_history"):
            ...
            try:
                # 执行对话
                with traced_span("agent.execute", attributes={"question_length": len(question)}):
                    async for ev in react.astream_events(
                        ...
                        if kind == "on_tool_start":
                            with traced_span(f"tool.{name}.start", attributes={"input": str(ev["data"].get("input", ""))[:200]}):
                                yield sse_event("tool_start", {"tool": name, "input": str(ev["data"].get("input", ""))[:200]})
                        elif kind == "on_tool_end":
                            with traced_span(f"tool.{name}.end", attributes={"output": str(ev["data"].get("output", ""))[:500]}):
            ...
            # 更新 last_active
            with traced_span("agent.update_last_active"):
